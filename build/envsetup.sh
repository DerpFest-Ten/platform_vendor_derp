...
function gerrit()
{
    if [ ! -d ".git" ]; then
        ...
        if [[ -z "${GERRIT_USER}" ]]; then
            git remote add gerrit $(git remote -v | grep -i "github\.com[:\/]DerpLab" | awk '{print $2}' | uniq | sed -e "s|.*github.com[:\/]DerpLab|ssh://review.derp.dev:29418/DERP|");
        else
            git remote add gerrit $(git remote -v | grep -i "github\.com[:\/]DerpLab" | awk '{print $2}' | uniq | sed -e "s|.*github.com[:\/]DerpLab|ssh://${GERRIT_USER}@review.derp.dev:29418/DERP|");
        fi
    fi
}
